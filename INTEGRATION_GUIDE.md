...
### Base URL
```
Production: https://fetch-fixer-1.preview.emergentagent.com/api
...
```bash
curl -X POST https://fetch-fixer-1.preview.emergentagent.com/api/auth/login \
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  https://fetch-fixer-1.preview.emergentagent.com/api/companies
```
...
receivers:
  - name: 'alert-whisperer'
    webhook_configs:
      - url: 'https://fetch-fixer-1.preview.emergentagent.com/api/webhooks/alerts'
        send_resolved: true
        http_config:
          headers:
...
#### Curl Example
```bash
curl -X POST https://fetch-fixer-1.preview.emergentagent.com/api/webhooks/alerts \
  -H "Content-Type: application/json" \
  -d '{
    "company_id": "comp-acme",
...
```
https://fetch-fixer-1.preview.emergentagent.com
```
