...
### Base URL
```
Production: https://connect-check-3.preview.emergentagent.com/api
...
```bash
curl -X POST https://connect-check-3.preview.emergentagent.com/api/auth/login \
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  https://connect-check-3.preview.emergentagent.com/api/companies
```
...
receivers:
  - name: 'alert-whisperer'
    webhook_configs:
      - url: 'https://connect-check-3.preview.emergentagent.com/api/webhooks/alerts'
        send_resolved: true
        http_config:
          headers:
...
```json
{
  "webhook_url": "https://connect-check-3.preview.emergentagent.com/api/webhooks/alerts",
  "payload": {
    "company_id": "comp-acme",
    "asset_name": "$HOSTNAME",
...
#### Curl Example
```bash
curl -X POST https://connect-check-3.preview.emergentagent.com/api/webhooks/alerts \
  -H "Content-Type: application/json" \
  -d '{
    "company_id": "comp-acme",
...
```
https://connect-check-3.preview.emergentagent.com
```
